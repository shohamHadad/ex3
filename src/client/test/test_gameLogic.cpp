...
TEST_F(test_gameLogic, possibleMovesCheck) {
	...
	movesX.push_back(Square(1,2));
	movesX.push_back(Square(2,1));
	movesX.push_back(Square(3,4));
	movesX.push_back(Square(4,3));
	vector<Square> movesO;
	movesO.push_back(Square(1,3));
	movesO.push_back(Square(2,4));
	movesO.push_back(Square(3,1));
	movesO.push_back(Square(4,2));

	// test for human player
	vector<Square> moves = gl.possibleMoves(x,o);
	for (unsigned int i = 0; i < movesX.size(); i++) {
		ASSERT_EQ(movesX[i], moves[i]);
	}
	moves.clear();
	moves = gl.possibleMoves(o,x);
	for (unsigned int i = 0; i < movesO.size(); i++) {
...
TEST_F(test_gameLogic, checkDirectionTest) {
	...
	// test for human player
	ASSERT_TRUE(gl.checkDirection(x,o,1,1,0,1,0));
	ASSERT_FALSE(gl.checkDirection(x,o,1,1,1,1,0));
	ASSERT_TRUE(gl.checkDirection(o,x,1,2,0,-1,0));
	ASSERT_FALSE(gl.checkDirection(o,x,1,2,1,-1,0));
...
TEST_F(test_gameLogic, flipInRightDirectionCheck) {
	...
	gl.flipInRightDirection(o, x, 1, 2, 1, -1);
